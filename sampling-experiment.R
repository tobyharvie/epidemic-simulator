...
prepare_data <- function (epi,sequences,ps,dir){
  ...
  dir.create(paste(dir,'ScITree/rinputs',sep=''))
  ...
  para.sf <- data.frame('alpha_sf' = 0.00015, # do some more work on this
                        ...
                        'tau_susc_sf' = 0.25,
                        'beta_m_sf' = 1)
  outdir <- paste(dir,"ScITree/rinputs/",sep='')
  ...
  writeLines(fasta_out, paste0(dir,"SCOTTI/seqs.fasta"))
  # sampling dates
  ...
  args <- c(
    ...
    "--fasta", paste0(dir,"SCOTTI/seqs.fasta"),
    "--dates", paste0(dir,"SCOTTI/dates.csv"),
    "--hosts", paste0(dir,"SCOTTI/hosts.csv"),
    "--hostTimes", paste0(dir,"SCOTTI/hostTimes.csv"),
    "--output", paste0(dir,'SCOTTI/scotti-ps-',ps),
  ...
  if (file.exists(paste0(dir,'SCOTTI/scotti-ps-',ps,'.xml'))) {
    #Delete file if it exists
    file.remove(paste0(dir,'SCOTTI/scotti-ps-',ps,'.xml'))
  }
...
for (ps in seq(from = 0.5, to = 1, by = 0.1)){

  print(paste('Simulating epidemic with ps=',ps))
  seed_used <- 20
  repeat{
    set.seed(seed_used)
    ...
    if (!is.na(epi[round(0.8*n_pop),2])) { 
      ...
      dir2 <- paste0(dir,"/ps-",ps*10,'/')
      dir.create(dir2)
      save(epi,newick,seed_used,file=paste0(dir2, '/epi-data'))
      
      print("Simulated epidemic:")
      print(epi)
      
      prepare_data(epi,sequences,ps,dir2)
...
# with ps=0.6 we had an empirical ps of 0.5, so we must redo this one. Am redoing this here as to not mess up the 
{
  ps=0.6
  print(paste('Simulating epidemic with ps=',ps))
  seed_used <- 900
